		...
		<table cellspacing="0" cellpadding="0" class="TableStyle">
			...
			<%-- <tbody id="TableData" class="dataContainer">
					...
					<tr align="CENTER" valign="MIDDLE" id="TableTitle">
						...
						<td>60</td>
						<td><s:a action="diameter_continueAlgorithm">显示模型|导出到Excel</s:a></td>
					</tr>
					<tr align="CENTER" valign="MIDDLE" id="TableTitle">
						<td>2</td>
						<td>1</td>
			...
			<!--显示数据列表-->
			<tbody id="TableData" class="dataContainer">
				<s:iterator value="#request.rms" status="status">
					<tr align="CENTER" valign="MIDDLE" id="TableTitle">
						<td>${reportId }</td>
						<td>${projectId}</td>
						<td>${fa }</td>
						<td>${investment}</td><!-- "project_openProject?id=%{id}" -->
						<td><s:a action="layout_showModel?reportId=%{reportId}" >显示模型</s:a>
						   <s:a action="layout_exportToExcel?id=%{id}" >导出到Excel</s:a>
						</td>
					</tr>
				</s:iterator>
			</tbody>
